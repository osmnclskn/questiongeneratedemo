...
class Question(BaseModel):
    ...
    reason: Optional[str] = Field(
        description="Explain why this question is significant for quality control concerning the topic provided."
    )
...
class PQMQualityControlQuestionGenerator:
    ...
    def generate_question(self, doc_content, old_questions):
        human_message_content = self.human_message_template.format(
            doc_content=doc_content, old_questions=old_questions
        )
        messages = [
